...
import type { ChangeInventoryItemStandardCostModel } from '../types/catalog.types';
import { catalogKeys } from './catalog.queries';

export const useChangeInventoryItemStandardCostMutation = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (command: ChangeInventoryItemStandardCostModel) => changeInventoryItemStandardCost(command.inventoryItemId, toChangeInventoryItemStandardCostRequestDto(command)),
    onSuccess: async () => {
