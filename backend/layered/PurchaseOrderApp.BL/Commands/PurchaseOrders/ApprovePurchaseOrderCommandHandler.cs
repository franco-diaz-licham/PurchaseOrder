...
{
    public Task<Result<PurchaseOrderResponse>> ExecuteAsync(ApprovePurchaseOrderCommand command, CancellationToken cancellationToken)
    {
        ...
        return purchaseOrderMutationCoordinator.ExecuteAsync(
            ...
            command.OccurredAt,
            (purchaseOrder, _) => {
                var rule = purchaseOrderPolicy.CanApprove(purchaseOrder);
