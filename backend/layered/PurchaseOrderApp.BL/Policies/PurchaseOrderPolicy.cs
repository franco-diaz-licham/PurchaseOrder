...
public sealed class PurchaseOrderPolicy
{
    ...
    {
        if (purchaseOrder.Status == PurchaseOrderStatus.Cancelled) {
            return Result.Fail("Cancelled purchase orders cannot be changed.");
        }

        if (purchaseOrder.Status == PurchaseOrderStatus.Closed) {
            return Result.Fail("Closed purchase orders cannot be changed.");
        }
    ...
    public Result CanReserve(PurchaseOrder purchaseOrder)
    {
        if (purchaseOrder.Status != PurchaseOrderStatus.Approved) {
            return Result.Fail("Only approved purchase orders can receive reservations.");
        }
    ...
    {
        if (purchaseOrder.Lines.Any(line => line.QuantityReserved > 0)) {
            return Result.Fail("Purchase orders with active reservations cannot be cancelled.");
        }
